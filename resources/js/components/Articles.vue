<template>
    <div>
        ...
        <div class="card card-body mt-4" v-for="article in articles" v-bind:key="article.id">
            <h3>{{ article.title }}</h3>
            <p>{{ article.body }}</p>
            <hr>
...
export default {
    ...
    methods: {
        fetchArticles(page_url) {
            let vm = this;
            page_url = page_url || 'http://127.0.0.1:8181/api/articles'
            fetch(page_url)
                .then(res => res.json())
                .then(res => {
                    console.log(res);
        ...
        deleteArticle(id) {
            if(confirm('Are you sure?')) {
                fetch(`http://127.0.0.1:8181/api/article/${id}`, {
                    method: 'delete'
                })
                .then(res => res.json())
                .then(data => {
        ...
        addArticle() {
            if(this.edit === false) {
                // Add 
                fetch('http://127.0.0.1:8181/api/article/', {
                    method: 'post',
